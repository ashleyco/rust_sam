...
impl Game {
    ...
    fn init(&mut self) {
        ...
        for x in 0..self.width {
            ...
            self.walls.push(Wall::new(x, (self.height as u32) - 1));
        }
        for y in 0..self.height {
            self.walls.push(Wall::new(0, y));
            self.walls.push(Wall::new((self.width as u32) - 1, y));
        }

        // add random walls
        for _ in 0..self.n_random_walls {
            let mut wall = Wall::default();
            wall.set_rand_position(&mut self.rng, (1..self.width - 1).into(), (1..self.height - 1).into());
