...
impl Game {
    ...
    fn init(&mut self) {
        ...
        for x in 0..self.width {
            ...
            self.walls.push(Wall::new(x, ((self.height as u32) - 1).try_into().unwrap()));
        }
        for y in 0..self.height {
            self.walls.push(Wall::new(0, y));
            self.walls.push(Wall::new(((self.width as u32) - 1).try_into().unwrap(), y));
        }
        ...
        self.enemies.iter_mut().for_each(|enemy| {
            enemy.set_rand_position(
                &mut self.rng,
                1.0..(self.width as f32) - 1.0,
                1.0..(self.height as f32) - 1.0,
            );
        });
            ...
            .any(|wall| wall.position() == self.collectible.position())
        {
            self.collectible.set_rand_position(
                &mut self.rng,
                1..(self.width as u32) - 1,
                1..(self.height as u32) - 1,      
            );
        }
    }
